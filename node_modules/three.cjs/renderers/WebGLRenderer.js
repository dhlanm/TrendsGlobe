...
var WebGLProgramModule = require('./webgl/WebGLProgram');
var UniformsUtilsModule = require('./shaders/UniformsUtils');
var ShaderLibModule = require('./shaders/ShaderLib');
var Matrix3Module = require('../math/Matrix3');
var LensFlarePluginModule = require('./webgl/plugins/LensFlarePlugin');
var SpritePluginModule = require('./webgl/plugins/SpritePlugin');
var ShadowMapPluginModule = require('./webgl/plugins/ShadowMapPlugin');
var WebGLExtensionsModule = require('./webgl/WebGLExtensions');
var WebGLStateModule = require('./webgl/WebGLState');
var Vector3Module = require('../math/Vector3');
var Matrix4Module = require('../math/Matrix4');
var FrustumModule = require('../math/Frustum');
...
WebGLRenderer = function (parameters) {
	...
	var _this = this, _programs = [],
		...
		_currentProgram = null, _currentFramebuffer = null, _currentMaterialId = -1, _currentGeometryProgram = '', _currentCamera = null, _usedTextureUnits = 0, _viewportX = 0, _viewportY = 0, _viewportWidth = _canvas.width, _viewportHeight = _canvas.height, _currentWidth = 0, _currentHeight = 0,
		// frustum
		_frustum = new FrustumModule.Frustum(),
		// camera matrices cache
		_projScreenMatrix = new Matrix4Module.Matrix4(), _vector3 = new Vector3Module.Vector3(),
		// light arrays cache
		_direction = new Vector3Module.Vector3(), _lightsNeedUpdate = true, _lights = {
			ambient: [
				0,
				0,
				0
	...
	var state = new WebGLStateModule.WebGLState(_gl, paramThreeToGL);
	...
	var extensions = new WebGLExtensionsModule.WebGLExtensions(_gl);
	...
	// Plugins
	var shadowMapPlugin = new ShadowMapPluginModule.ShadowMapPlugin(this, lights, _webglObjects, _webglObjectsImmediate);
	var spritePlugin = new SpritePluginModule.SpritePlugin(this, sprites);
	var lensFlarePlugin = new LensFlarePluginModule.LensFlarePlugin(this, lensFlares);
	// API
	...
	var deallocateRenderTarget = function (renderTarget) {
		...
		if (!!(instance = renderTarget) && !!instance.isWebGLRenderTargetCube) {
			for (var i = 0; i < 6; i++) {
	...
	function getBufferMaterial(object, geometryGroup) {
		return !!(instance = object.material) && !!instance.isMeshFaceMaterial ? object.material.materials[geometryGroup.materialIndex] : object.material;
	}
	function materialNeedsFaceNormals(material) {
		return (!!(instance = material) && !!instance.isMeshPhongMaterial) === false && material.shading === Three.FlatShading;
	}
	// Buffer setting
	...
	this.renderBufferImmediate = function (object, program, material) {
		...
		if (object.hasNormals) {
			...
			if ((!!(instance = material) && !!instance.isMeshPhongMaterial) === false && material.shading === Three.FlatShading) {
	...
	this.renderBufferDirect = function (camera, lights, fog, material, geometry, object) {
		...
		// render mesh
		if (!!(instance = object) && !!instance.isMesh) {
			var mode = material.wireframe === true ? _gl.LINES : _gl.TRIANGLES;
			var index = geometry.attributes.index;
			if (index) {
				// indexed triangles
				var type, size;
				if (toString.call(index.array).slice(8, -1) === 'Uint32Array' && extensions.get('OES_element_index_uint')) {
		...
		} else if (!!(instance = object) && !!instance.isPointCloud) {
			// render particles
			var mode = _gl.POINTS;
			var index = geometry.attributes.index;
			if (index) {
		...
		} else if (!!(instance = object) && !!instance.isLine) {
			...
			if (index) {
				// indexed lines
				var type, size;
				if (toString.call(index.array).slice(8, -1) === 'Uint32Array') {
					type = _gl.UNSIGNED_INT;
					size = 4;
				} else {
					type = _gl.UNSIGNED_SHORT;
	...
	this.render = function (scene, camera, renderTarget, forceClear) {
		if ((!!(instance = camera) && !!instance.isCamera) === false) {
		...
		scene.traverse(function (object) {
			if (!!(instance = object) && !!instance.isSkinnedMesh) {
	...
	function projectObject(object) {
		if (object.visible === false)
			return;
		if (!!(instance = object) && !!instance.isScene || !!(instance = object) && !!instance.isGroup) {
		} else {
			initObject(object);
			if (!!(instance = object) && !!instance.isLight) {
				lights.push(object);
			} else if (!!(instance = object) && !!instance.isSprite) {
				sprites.push(object);
			} else if (!!(instance = object) && !!instance.isLensFlare) {
	...
	function renderObjects(renderList, camera, lights, fog, overrideMaterial) {
		...
		for (var i = 0, l = renderList.length; i < l; i++) {
			...
			if (!!(instance = buffer) && !!instance.isBufferGeometry) {
	...
	function unrollBufferMaterial(globject) {
		...
		var geometry = object.geometry;
		var material = object.material;
		if (!!(instance = material) && !!instance.isMeshFaceMaterial) {
			var materialIndex = !!(instance = geometry) && !!instance.isBufferGeometry ? 0 : buffer.materialIndex;
	...
	function initObject(object) {
		if (object.__webglInit === undefined) {
			...
			object._modelViewMatrix = new Matrix4Module.Matrix4();
			object._normalMatrix = new Matrix3Module.Matrix3();
		...
		if (geometry === undefined) {
			...
			if (!!(instance = geometry) && !!instance.isBufferGeometry) {
				_this.info.memory.geometries++;
			} else if (!!(instance = object) && !!instance.isMesh) {
		...
		if (object.__webglActive === undefined) {
			...
			} else if (!!(instance = object) && !!instance.isLine || !!(instance = object) && !!instance.isPointCloud) {
				addBuffer(_webglObjects, geometry, object);
			} else if (!!(instance = object) && !!instance.isImmediateRenderObject || object.immediateRenderCallback) {
	...
	function initGeometryGroups(object, geometry) {
		...
		if (geometryGroups[geometry.id] === undefined || geometry.groupsNeedUpdate === true) {
			...
			geometryGroups[geometry.id] = makeGroups(geometry, !!(instance = material) && !!instance.isMeshFaceMaterial);
		...
		if (!!(instance = geometry) && !!instance.isBufferGeometry) {
			...
			for (var i = 0, l = attributesKeys.length; i < l; i++) {
				...
				if (attribute.buffer === undefined) {
					...
					_gl.bufferData(bufferType, attribute.array, !!(instance = attribute) && !!instance.isDynamicBufferAttribute ? _gl.DYNAMIC_DRAW : _gl.STATIC_DRAW);
	...
	function removeObject(object) {
		if (!!(instance = object) && !!instance.isMesh || !!(instance = object) && !!instance.isPointCloud || !!(instance = object) && !!instance.isLine) {
			delete _webglObjects[object.id];
		} else if (!!(instance = object) && !!instance.isImmediateRenderObject || object.immediateRenderCallback) {
	...
	function initMaterial(material, lights, fog, object) {
		...
		if (shaderID) {
			var shader = ShaderLibModule.ShaderLib[shaderID];
			material.__webglShader = {
				uniforms: UniformsUtilsModule.UniformsUtils.clone(shader.uniforms),
		...
		var parameters = {
			...
			fog: fog,
			useFog: material.fog,
			fogExp: !!(instance = fog) && !!instance.isFogExp2,
		...
		if (program === undefined) {
			program = new WebGLProgramModule.WebGLProgram(_this, code, material, parameters);
	...
	function setProgram(camera, lights, fog, material, object) {
		...
		if (refreshProgram || camera !== _currentCamera) {
			...
			if (!!(instance = material) && !!instance.isShaderMaterial || !!(instance = material) && !!instance.isMeshPhongMaterial || material.envMap) {
				if (p_uniforms.cameraPosition !== null) {
					_vector3.setFromMatrixPosition(camera.matrixWorld);
					_gl.uniform3f(p_uniforms.cameraPosition, _vector3.x, _vector3.y, _vector3.z);
				}
			}
			if (!!(instance = material) && !!instance.isMeshPhongMaterial || !!(instance = material) && !!instance.isMeshLambertMaterial || !!(instance = material) && !!instance.isMeshBasicMaterial || !!(instance = material) && !!instance.isShaderMaterial || material.skinning) {
		...
		if (refreshMaterial) {
			...
			if (!!(instance = material) && !!instance.isMeshPhongMaterial || !!(instance = material) && !!instance.isMeshLambertMaterial || material.lights) {
			...
			if (!!(instance = material) && !!instance.isMeshBasicMaterial || !!(instance = material) && !!instance.isMeshLambertMaterial || !!(instance = material) && !!instance.isMeshPhongMaterial) {
				refreshUniformsCommon(m_uniforms, material);
			}
			// refresh single material specific uniforms
			if (!!(instance = material) && !!instance.isLineBasicMaterial) {
				refreshUniformsLine(m_uniforms, material);
			} else if (!!(instance = material) && !!instance.isLineDashedMaterial) {
				refreshUniformsLine(m_uniforms, material);
				refreshUniformsDash(m_uniforms, material);
			} else if (!!(instance = material) && !!instance.isPointCloudMaterial) {
				refreshUniformsParticle(m_uniforms, material);
			} else if (!!(instance = material) && !!instance.isMeshPhongMaterial) {
				refreshUniformsPhong(m_uniforms, material);
			} else if (!!(instance = material) && !!instance.isMeshLambertMaterial) {
				refreshUniformsLambert(m_uniforms, material);
			} else if (!!(instance = material) && !!instance.isMeshDepthMaterial) {
				m_uniforms.mNear.value = camera.near;
				m_uniforms.mFar.value = camera.far;
				m_uniforms.opacity.value = material.opacity;
			} else if (!!(instance = material) && !!instance.isMeshNormalMaterial) {
				m_uniforms.opacity.value = material.opacity;
			}
	...
	function refreshUniformsCommon(uniforms, material) {
		...
		uniforms.flipEnvMap.value = !!(instance = material.envMap) && !!instance.isWebGLRenderTargetCube ? 1 : -1;
	...
	function refreshUniformsFog(uniforms, fog) {
		...
		if (!!(instance = fog) && !!instance.isFog) {
			uniforms.fogNear.value = fog.near;
			uniforms.fogFar.value = fog.far;
		} else if (!!(instance = fog) && !!instance.isFogExp2) {
	...
	function refreshUniformsShadow(uniforms, lights) {
		if (uniforms.shadowMatrix) {
			...
			for (var i = 0, il = lights.length; i < il; i++) {
				...
				if (!light.castShadow)
					continue;
				if (!!(instance = light) && !!instance.isSpotLight || !!(instance = light) && !!instance.isDirectionalLight && !light.shadowCascade) {
	...
	function loadUniformsGeneric(uniforms) {
		...
		for (var j = 0, jl = uniforms.length; j < jl; j++) {
			...
			case 't':
				...
				if (!texture)
					continue;
				if (!!(instance = texture) && !!instance.isCubeTexture || toString.call(texture.image).slice(8, -1) === 'Array' && texture.image.length === 6) {
					// CompressedTexture can have Array in image :/
					setCubeTexture(texture, textureUnit);
				} else if (!!(instance = texture) && !!instance.isWebGLRenderTargetCube) {
	...
	function setupLights(lights) {
		...
		for (l = 0, ll = lights.length; l < ll; l++) {
			...
			if (!!(instance = light) && !!instance.isAmbientLight) {
				if (!light.visible)
					continue;
				r += color.r;
				g += color.g;
				b += color.b;
			} else if (!!(instance = light) && !!instance.isDirectionalLight) {
				dirCount += 1;
				if (!light.visible)
					continue;
				...
				dirLength += 1;
			} else if (!!(instance = light) && !!instance.isPointLight) {
				pointCount += 1;
				if (!light.visible)
					continue;
				...
				pointLength += 1;
			} else if (!!(instance = light) && !!instance.isSpotLight) {
				spotCount += 1;
				if (!light.visible)
					continue;
				...
				spotLength += 1;
			} else if (!!(instance = light) && !!instance.isHemisphereLight) {
				hemiCount += 1;
				if (!light.visible)
					continue;
				_direction.setFromMatrixPosition(light.matrixWorld);
	...
	this.uploadTexture = function (texture) {
		...
		var image = texture.image, isImagePowerOfTwo = MathModule.Math.isPowerOfTwo(image.width) && MathModule.Math.isPowerOfTwo(image.height), glFormat = paramThreeToGL(texture.format), glType = paramThreeToGL(texture.type);
		setTextureParameters(_gl.TEXTURE_2D, texture, isImagePowerOfTwo);
		var mipmap, mipmaps = texture.mipmaps;
		if (!!(instance = texture) && !!instance.isDataTexture) {
		...
		} else if (!!(instance = texture) && !!instance.isCompressedTexture) {
			for (var i = 0, il = mipmaps.length; i < il; i++) {
				mipmap = mipmaps[i];
	...
	function setCubeTexture(texture, slot) {
		if (texture.image.length === 6) {
			if (texture.needsUpdate) {
				...
				_gl.pixelStorei(_gl.UNPACK_FLIP_Y_WEBGL, texture.flipY);
				var isCompressed = !!(instance = texture) && !!instance.isCompressedTexture;
				var isDataTexture = !!(instance = texture.image[0]) && !!instance.isDataTexture;
				var cubeImage = [];
				for (var i = 0; i < 6; i++) {
				...
				var image = cubeImage[0], isImagePowerOfTwo = MathModule.Math.isPowerOfTwo(image.width) && MathModule.Math.isPowerOfTwo(image.height), glFormat = paramThreeToGL(texture.format), glType = paramThreeToGL(texture.type);
	...
	this.setRenderTarget = function (renderTarget) {
		var isCube = !!(instance = renderTarget) && !!instance.isWebGLRenderTargetCube;
		if (renderTarget && renderTarget.__webglFramebuffer === undefined) {
			...
			var isTargetPowerOfTwo = MathModule.Math.isPowerOfTwo(renderTarget.width) && MathModule.Math.isPowerOfTwo(renderTarget.height), glFormat = paramThreeToGL(renderTarget.format), glType = paramThreeToGL(renderTarget.type);
			if (isCube) {
	...
	this.readRenderTargetPixels = function (renderTarget, x, y, width, height, buffer) {
		if (!(!!(instance = renderTarget) && !!instance.isWebGLRenderTarget)) {
	...
	function allocateBones(object) {
		if (_supportsBoneTextures && object && object.skeleton && object.skeleton.useVertexTexture) {
			...
			if (object !== undefined && (!!(instance = object) && !!instance.isSkinnedMesh)) {
	...
	function allocateLights(lights) {
		...
		for (var l = 0, ll = lights.length; l < ll; l++) {
			...
			if (light.onlyShadow || light.visible === false)
				continue;
			if (!!(instance = light) && !!instance.isDirectionalLight)
				dirLights++;
			if (!!(instance = light) && !!instance.isPointLight)
				pointLights++;
			if (!!(instance = light) && !!instance.isSpotLight)
				spotLights++;
			if (!!(instance = light) && !!instance.isHemisphereLight)
				hemiLights++;
		}
		return {
	...
	function allocateShadows(lights) {
		...
		for (var l = 0, ll = lights.length; l < ll; l++) {
			...
			if (!!(instance = light) && !!instance.isSpotLight)
				maxShadows++;
			if (!!(instance = light) && !!instance.isDirectionalLight && !light.shadowCascade)
				maxShadows++;
		}
		return maxShadows;
	}
