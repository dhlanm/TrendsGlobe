...
AnimationHandler = {
	...
	init: function (data) {
		...
		for (var h = 0; h < data.hierarchy.length; h++) {
			for (var k = 0; k < data.hierarchy[h].keys.length; k++) {
				...
				// create quaternions
				if (data.hierarchy[h].keys[k].rot !== undefined && !(!!(instance = data.hierarchy[h].keys[k].rot) && !!instance.isQuaternion)) {
					var quat = data.hierarchy[h].keys[k].rot;
					data.hierarchy[h].keys[k].rot = new QuaternionModule.Quaternion().fromArray(quat);
				}
			}
	...
	parse: function (root) {
		...
		// setup hierarchy
		var hierarchy = [];
		if (!!(instance = root) && !!instance.isSkinnedMesh) {
