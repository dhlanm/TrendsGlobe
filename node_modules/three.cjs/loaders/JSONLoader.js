...
JSONLoader = function (showStatus) {
	LoaderModule.Loader.call(this, showStatus);
	this.withCredentials = false;
};
JSONLoader.prototype = Object.create(LoaderModule.Loader.prototype);
...
JSONLoader.prototype.parse = function (json, texturePath) {
	var geometry = new GeometryModule.Geometry(), scale = json.scale !== undefined ? 1 / json.scale : 1;
	parseModel(scale);
	parseSkin();
	parseMorphing(scale);
	...
	function parseModel(scale) {
		...
		while (offset < zLength) {
			vertex = new Vector3Module.Vector3();
		...
		while (offset < zLength) {
			...
			if (isQuad) {
				faceA = new Face3Module.Face3();
				faceA.a = faces[offset];
				faceA.b = faces[offset + 1];
				faceA.c = faces[offset + 3];
				faceB = new Face3Module.Face3();
				...
				if (hasFaceVertexUv) {
					for (i = 0; i < nUvLayers; i++) {
						...
						for (j = 0; j < 4; j++) {
							...
							uv = new Vector2Module.Vector2(u, v);
							if (j !== 2)
				...
				if (hasFaceVertexNormal) {
					for (i = 0; i < 4; i++) {
						normalIndex = faces[offset++] * 3;
						normal = new Vector3Module.Vector3(normals[normalIndex++], normals[normalIndex++], normals[normalIndex]);
						if (i !== 2)
				...
				if (hasFaceVertexColor) {
					for (i = 0; i < 4; i++) {
						...
						hex = colors[colorIndex];
						if (i !== 2)
							faceA.vertexColors.push(new ColorModule.Color(hex));
						if (i !== 0)
							faceB.vertexColors.push(new ColorModule.Color(hex));
					}
				}
				geometry.faces.push(faceA);
				geometry.faces.push(faceB);
			} else {
				face = new Face3Module.Face3();
				face.a = faces[offset++];
				face.b = faces[offset++];
				face.c = faces[offset++];
				if (hasMaterial) {
				...
				if (hasFaceVertexColor) {
					for (i = 0; i < 3; i++) {
						colorIndex = faces[offset++];
						face.vertexColors.push(new ColorModule.Color(colors[colorIndex]));
					}
				}
	...
	function parseSkin() {
		...
		if (json.skinWeights) {
			for (var i = 0, l = json.skinWeights.length; i < l; i += influencesPerVertex) {
				...
				geometry.skinWeights.push(new Vector4Module.Vector4(x, y, z, w));
			}
		}
		if (json.skinIndices) {
			for (var i = 0, l = json.skinIndices.length; i < l; i += influencesPerVertex) {
				...
				geometry.skinIndices.push(new Vector4Module.Vector4(a, b, c, d));
			}
		}
		...
		if (json.morphColors !== undefined) {
			...
			for (i = 0, l = json.morphColors.length; i < l; i++) {
				...
				for (c = 0, cl = srcColors.length; c < cl; c += 3) {
					color = new ColorModule.Color(16755200);
...
JSONLoader.prototype.isJSONLoader = true;
